...
int scope(int ts){ //funzione scope, ritorna posizione esatta di dove inserire (e,ts)
    ...
    while (l>=MM) //avendo MM celle, la posizione della mia finestra sarà sempre al suo interno
        l -=MM;
    if (x->N-x->M==0) {
        ...
        if (o<=ts) {
            while (o <= ts) {
                ...
                if (x->N >= MM) //se l'ultima casella supera il l'ampiezza della tabella, si ritorna da zero
                    x->N = x->N - MM;
                if (x->count>MM) {//cioè sono state messe più finestre
                    x->head[m]->c=NULL; // pulisco la cella in caso fosse precedentemente piena
                    x->head[m]->nc=0;
                   // evictWindow(ts,m);
                    x->head[m]->w.o = o;
                    x->head[m]->w.c = o + width;
                }
                else
                    x->head[m]=newNode(o, o + width);
                o += slide;
                m++;
                if (m >= MM) //il contatore riparte dalla prima casella
                    m -= MM;
            }
        }
    }
